...
- 需求详情页必须区分流程推进按钮、生成指令按钮和刷新按钮：流程推进按钮是实心胶囊确认按钮；生成按钮是白底描边按钮，和流程确认按钮同处标题状态区但视觉上明显区分；刷新按钮使用 `el-icon-refresh` 的白底描边按钮并只负责重新读取详情和资料包。前端只复制后端返回指令，不在页面展开展示指令内容。`confirmed` 待执行开发阶段不展示生成执行指令，指定开发人员点击开始开发进入 `developing` 后才展示生成执行指令；`developing` 和 `repairing` 在执行报告与 Review 报告未回写齐全前只展示生成任务指令，产物回写齐全后隐藏生成任务指令并展示提交验收或提交返修验收。`plan_ready` 需求设计待确认状态下，需求创建人除了确认需求设计，还必须能打开“补充调整说明”输入区；输入区展开后隐藏确认需求设计按钮，提交后回到 `plan_pending`，由指定开发人员按新说明重新生成需求设计，支持多轮迭代。
...
- 需求列表和详情必须展示指定开发人员；非管理员只能看到后端返回的参与人范围内需求，前端按钮也必须避免让其他开发人员看到不属于自己的流转入口。需求列表不得同时展示同阶段生成指令和反馈结论入口；`submitted`、`plan_pending` 在列表缺少资料包版本明细时优先隐藏反馈结论动作，详情页则按 `requirement_assessment` 或 `requirement` 是否已回写，在生成指令和反馈结论之间二选一展示。详情页 `developing` 和 `repairing` 同样不得同时展示生成任务指令和验收提交按钮，必须等待 `execution_report` 与 `review_report` 都已回写后才展示验收提交入口。
