...
- 需求列表和详情必须展示指定开发人员；非管理员只能看到后端返回的参与人范围内需求，前端按钮也必须避免让其他开发人员看到不属于自己的流转入口。需求列表不得同时展示同阶段生成指令和反馈结论入口；`submitted`、`plan_pending` 在列表缺少资料包版本明细时优先隐藏反馈结论动作，详情页则按 `requirement_assessment` 或 `requirement` 是否已回写，在生成指令和反馈结论之间二选一展示。
