...
- Agent 交接资料在需求详情和 `demandId` 聚焦模式下使用只读 Markdown 阅读态展示，必须先转义 HTML 再渲染标题、列表、引用、代码块等常见 Markdown；这两个只读入口都提供刷新按钮，用于 MCP 回写后重新拉取需求状态和资料包版本；独立管理模式可保留 textarea 保存能力，不引入 Markdown 编辑器依赖。
...
- 需求列表操作列不展示 Agent 交接资料入口；需求详情页直接内嵌当前需求的 Agent 交接资料包，以当前需求标题为区块标题，一级标签只展示需求草稿、需求可行性评估、需求设计、执行计划、执行报告和 Review 报告等业务文档标签，并按状态和已回写产物默认切换到当前阶段相关标签；补充说明不单独显示标签，需求人补充记录折叠展示在需求可行性评估标签内，需求设计调整记录折叠展示在需求设计标签内，标签正文不限制高度；不再额外重复展示一组独立的需求设计/执行计划预览；详情页不展示协作工具栏和指令内容预览，只在标题状态区按当前阶段展示一个独立生成按钮。
- 需求删除按钮只使用 `req:demand:remove` 展示，预期仅管理员可见；需求人员和开发人员不可见删除入口。
- 以 `demandId` 上下文打开 Agent 交接资料页时，页面必须以当前需求为上下文，只展示需求标题、刷新入口和各类文档内容；不得展示需求 ID 查询、手动生成资料、加载最新或保存新版本等管理动作。
- 需求详情页必须区分流程推进按钮、生成指令按钮和刷新按钮：流程推进按钮是实心胶囊确认按钮；生成按钮是白底描边按钮，和流程确认按钮同处标题状态区但视觉上明显区分；刷新按钮使用 `el-icon-refresh` 的白底描边按钮并只负责重新读取详情和资料包。前端只复制后端返回指令，不在页面展开展示指令内容。`confirmed` 待执行开发阶段不展示生成执行指令，指定开发人员点击开始开发进入 `developing` 后才展示生成执行指令；`developing` 在执行报告与 Review 报告未回写齐全前只展示生成执行任务指令，产物回写齐全后隐藏生成指令并展示提交验收；`repairing` 必须先展示生成返修任务指令，只有最新“需求人返修问题说明”之后回写了新的执行报告与 Review 报告，才隐藏生成返修任务指令并展示提交返修验收；`closeout_pending` 未通过平台归档验证前只展示生成合并归档指令，验证通过后隐藏生成合并归档指令并展示确认归档完成。`plan_ready` 需求设计待确认状态下，需求创建人除了确认需求设计，还必须能打开“补充调整说明”输入区；输入区展开后隐藏确认需求设计按钮，提交后回到 `plan_pending`，由指定开发人员按新说明重新生成需求设计，支持多轮迭代。
- 需求设计生成指令只在 `plan_pending` 阶段展示；`plan_ready` 是需求设计待确认阶段，不展示“生成需求设计指令”。
...
- 流程按钮由 `status.js` 统一定义，并同时按角色、`req:demand:edit` 按钮权限和当前行参与人过滤：需求创建人可提交需求、提交补充说明、确认需求设计、带问题说明提交返修和确认验收；指定开发人员可提交需求分析结论、需求设计结论、开始开发、提交验收、提交返修验收和确认归档完成；管理员角色可见全部流程按钮。
...
- 需求列表和详情必须展示指定开发人员；非管理员只能看到后端返回的参与人范围内需求，前端按钮也必须避免让其他开发人员看到不属于自己的流转入口。需求列表不得同时展示同阶段生成指令和反馈结论入口；`submitted`、`plan_pending` 在列表缺少资料包版本明细时优先隐藏反馈结论动作，详情页则按 `requirement_assessment` 或 `requirement` 是否已回写，在生成指令和反馈结论之间二选一展示。需求列表 `repairing` 行不得直接展示“提交返修验收”，开发人员必须进入详情页先复制返修任务指令；详情页 `developing` 和 `repairing` 同样不得同时展示生成任务指令和验收提交按钮，`developing` 必须等待 `execution_report` 与 `review_report` 都已回写后才展示验收提交入口，`repairing` 必须等待最新返修问题说明之后的新 `execution_report` 与 `review_report` 都已回写后才展示返修验收提交入口；`closeout_pending` 不得同时展示生成合并归档指令和确认归档完成，必须等待 `/requirement/demand/{demandId}/closeout-verification` 返回 `verified=true` 后才展示确认归档完成。
- 待验收状态必须同时提供“提交返修”和“确认验收”；点击“提交返修”必须打开返修问题说明弹窗，输入非空问题说明后调用 `/requirement/demand/{demandId}/repair`，不得直接调用普通状态接口；返修中状态提交后回到待验收；确认验收后进入待合并归档，详情页应通过 `/requirement/package/{demandId}` 展示需求可行性评估、需求设计、执行计划、执行报告、Review 报告和需求人返修问题说明等历史版本，用于判断返修轮次。
