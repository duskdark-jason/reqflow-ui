...
- 流程按钮由 `status.js` 统一定义并按角色过滤：需求人员可提交需求、确认需求设计、提交返修和确认验收；开发人员可提交需求设计、开始开发、提交验收和提交返修验收；管理员角色可见全部流程按钮。
