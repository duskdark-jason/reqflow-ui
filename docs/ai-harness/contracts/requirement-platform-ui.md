...
- `review` 状态必须提供“提交返修”和“确认验收”两个流程按钮；`repairing` 状态提供“提交返修验收”并流转回 `review`；确认验收后进入 `closeout_pending`，详情页读取 `/requirement/demand/{demandId}/closeout-verification` 判断平台归档验证结果。
