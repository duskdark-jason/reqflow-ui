...
- 详情页读取 `/requirement/package/{demandId}` 内嵌展示当前需求的 Agent 交接资料包，后端允许 `req:demand:query` 读取。资料包区块以当前需求标题为标题，按 artifact 类型只展示需求草稿、补充说明、需求可行性评估、需求设计、执行计划、执行报告和 Review 报告等业务文档的最新内容，并展示每类产物最近历史版本；不得再在详情底部额外重复展示一组独立的需求设计/执行计划预览。没有资料时展示空状态，不阻断页面打开。保存 artifact 必须追加版本，返修轮次依赖历史版本链判断。
- Agent 交接资料包默认标签必须跟随需求阶段：`draft/submitted` 默认需求草稿，`supplement_required` 默认补充说明，`plan_pending/rejected` 默认需求可行性评估，`plan_ready` 默认需求设计，`confirmed/developing` 默认执行计划，`review` 默认执行报告，`repairing/completed/archived` 默认 Review 报告。
