...
- 后端返回的 `codexSetupPackage` 是推荐复制给 Codex 的安装指令包，只在创建和重置结果弹窗中格式化 JSON 展示和复制。安装包包含 MCP 配置摘要、全局 skill 包、短提示词和 server metadata；页面不得向该包追加明文 Key、`plainKey` 或一次性 `actionToken`。
