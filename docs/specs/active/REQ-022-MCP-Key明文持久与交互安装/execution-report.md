...
| `src/views/requirement/demand/status.js`、`index.vue`、`detail.vue`、`scripts/test-demand-ui-helpers.js` | 修复需求列表/详情同阶段操作入口互斥，列表页不同时展示生成指令和反馈结论，详情页按资料包回写状态切换；开发和返修阶段必须等执行报告与 Review 报告回写后才展示验收提交入口。 |
...
| L2 | AC-010、AC-011 | `node scripts/test-demand-ui-helpers.js` | 通过，先红于开发结果产物门禁 helper 缺失，修复后通过 |
| L2 | AC-006 | `sh scripts/test-check-harness.sh` | 通过 |
| L1 | AC-001、AC-002、AC-003、AC-004、AC-010、AC-011 | `npm run build:prod` | 通过，存在历史体积告警 |
| L0 | AC-005、AC-006、AC-007、AC-008、AC-009、AC-010、AC-011 | `sh scripts/check-docs.sh && sh scripts/check-harness.sh complete --spec docs/specs/active/REQ-022-MCP-Key明文持久与交互安装` | 通过 |
