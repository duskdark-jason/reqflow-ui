...
| AC-011 | 历史版本和阶段指令入口 | `detail.vue`、`demand.js` | 详情页读取执行包版本列表，展示需求可行性评估等 artifact，并按当前状态展示需求分析、需求设计、执行任务或返修任务指令入口 | 通过 |
| AC-012 | actionToken 复制边界 | `project/maintain.vue`、`project/knowledge.vue`、`detail.vue`、UI 契约文档 | 后端 companion 单测确认指令内容只包含当前阶段工具、阶段有效和流转后失效；代码复核前端不拼接 actionToken | 通过 |
...
| AC-015 | 指令和状态文案拆分 | `detail.vue`、`status.js` | 文案区分“生成需求分析指令”“生成需求设计指令”“生成执行任务指令”和“生成返修任务指令”，流程确认按钮与生成按钮样式区分，页面不展示协作工具栏 | 通过 |
...
- AC-011：通过，详情页展示需求可行性评估、需求设计和执行包历史版本，并按角色和状态提供需求分析、需求设计、执行任务和返修任务指令复制入口。
...
- AC-015：通过，生成需求分析、需求设计、执行任务和返修任务文案已拆分。
...
- AC-019：通过，首页快捷入口按权限过滤，需求人员无 MCP 管理快捷入口，研发人员入口展示“查看需求”。
