...
4. 需求列表与详情：列表操作列移除 Agent 资料；详情页内嵌 Agent 交接资料包、按阶段展示需求分析、需求设计、执行任务、返修任务和合并归档指令入口；复制指令只使用后端返回内容；详情上下文的 Agent 交接资料只读展示当前需求文档，覆盖 AC-005、AC-011、AC-012、AC-013、AC-014、AC-026。
...
8. 指令与资料包再调整：将详情页复制按钮文案改为生成需求分析、需求设计、执行任务、返修任务和合并归档指令，资料包区块标题使用当前需求标题，按文档分组展示并修复底部重叠，覆盖 AC-015、AC-016、AC-026。
...
12. 执行指令阶段收紧：`confirmed` 阶段只展示开始开发，进入 `developing` 后才展示执行指令；补充调整后由后端强制重新生成需求设计，前端按状态刷新按钮，覆盖 AC-023。
13. MCP Key 管理和项目模块选择：前端保持普通用户绑定自己、安装指令展示和模块下拉前端页面优先选择，覆盖 AC-024、AC-025。
14. 合并归档：新增 `closeout_pending` 状态文案、默认 Review 报告标签、合并归档指令按钮和确认归档完成流程按钮，覆盖 AC-026。
...
| AC-023 | 开始开发后才展示执行指令、补充调整后重新生成设计 | `node scripts/test-demand-ui-helpers.js`、后端 companion 单测 |
| AC-024 | MCP Key 管理安装指令体验 | `npm run build:prod`、代码静态复核、后端 companion 单测 |
| AC-025 | 前端页面/菜单模块优先选择 | `node scripts/test-demand-ui-helpers.js`、代码静态复核 |
| AC-026 | 待合并归档阶段和合并归档指令入口 | `node scripts/test-demand-ui-helpers.js`、`npm run build:prod`、后端 companion 单测 |
