...
4. 需求列表与详情：列表操作列移除 Agent 资料；详情页内嵌 Agent 交接资料包、执行任务指令入口和资料版本记录；复制指令只使用后端返回内容；详情上下文的 Agent 交接资料只读展示当前需求文档，覆盖 AC-005、AC-011、AC-012、AC-013、AC-014。
...
7. 角色菜单与按钮体验：更新前端文档说明需求人员和开发人员菜单可见性，并让列表和详情流程按钮按角色过滤，确保详情页资料包读取依赖需求详情而非独立 Agent 资料菜单入口，覆盖 AC-017。
8. 指令与资料包再调整：将详情页复制按钮文案改为生成需求设计和执行任务，资料包区块标题使用当前需求标题，按文档分组展示并修复底部重叠，覆盖 AC-015、AC-016。
9. 需求填报附件：维护页增加需求来源必填、业务背景富文本粘贴图片和附件上传；详情页展示来源、富文本背景和附件，覆盖 AC-018。
10. 文档同步：更新模块文档和契约文档，覆盖 AC-007、AC-017、AC-018。
...
| AC-011 | 历史版本和执行任务指令 | 浏览器详情冒烟、接口冒烟 |
...
| AC-018 | 需求来源、背景图片和附件上传 | `npm run build:prod`、代码静态复核、浏览器新增/详情页冒烟 |
