...
2. 向导组件拆分：在项目管理目录下新增初始化向导组件，内部使用 Element UI steps、form、table 和 summary 区块，覆盖 AC-UI-001、AC-UI-002。
3. 项目列表改造：调整 `project/index.vue` 的新增和编辑操作，打开初始化向导；列表展示初始化状态和“继续初始化/接入中心”入口，覆盖 AC-UI-001、AC-UI-004。
4. 仓库步骤：默认生成前端仓库和后端仓库行，支持增删仓库、选择仓库类型、填写 Git 地址和默认分支，覆盖 AC-UI-002、AC-UI-003。
5. 客户基线步骤：支持维护客户线名称、编码、客户名称、统一基线分支和分支策略，覆盖 AC-UI-002。
6. 模块初始化步骤：展示模块摘要、索引摘要和 MCP 下一步指引，明确不保存本机目录，覆盖 AC-UI-003、AC-UI-004。
...
| 新增 | `src/views/requirement/project/components/ProjectInitWizard.vue` | 项目初始化分步向导 |
...
- L3 运行态冒烟：启动前端后打开项目管理，验证新增向导、编辑回显、仓库步骤、客户基线步骤、模块初始化步骤和 console/network。
...
| 验收 ID | 计划阶段 | 验证方式 |
|---|---|---|
| AC-UI-001 | 向导组件拆分、项目列表改造 | 页面冒烟和构建 |
| AC-UI-002 | 仓库步骤、客户基线步骤 | 页面交互和跨端联调 |
| AC-UI-003 | 仓库步骤、模块初始化步骤 | 文案检查和页面冒烟 |
| AC-UI-004 | 项目列表改造、模块初始化步骤 | 页面冒烟和接口联调 |
...
- 前端不得提交或保存个人本机绝对路径。
- 保留现有项目接入中心详情页，初始化向导负责首次创建和编辑主流程，详情页负责后续维护和索引查看。
