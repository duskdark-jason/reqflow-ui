...
适用场景：当前仓库正在建设需求管理平台本身，需求平台 MCP 能力尚未完整可用，或用户明确要求先本地维护需求平台自身的 harness、流程和业务实现。普通业务项目未接入 MCP 时，不使用本模式，改用 `local-harness-workflow.md` 中的 `本地 Harness 模式`。
...
- 平台自身不可用、MCP 不可用、Key 无法解析，或平台返回的远端/分支与当前工作区冲突时，必须停止平台驱动流程，说明原因；普通业务项目退回 `本地 Harness 模式`，需求平台自身项目才可退回 `平台自身建设模式`。
