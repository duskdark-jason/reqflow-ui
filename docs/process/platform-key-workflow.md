...
适用场景：当前仓库正在建设需求管理平台本身、平台类治理能力，或明确拷贝平台建设版本进行本地自举，且需求平台 MCP 能力尚未完整可用，或用户明确要求先本地维护平台建设相关 harness、流程和业务实现。普通业务项目仅仅未接入 MCP 时，不使用本模式，改用 `local-harness-workflow.md` 中的 `本地 Harness 模式`。
...
- 不能把平台自身建设模式作为所有普通业务项目未接入 MCP 的默认降级；只有项目明确采用平台建设版本、自举平台能力或建设平台类治理能力时才使用。
...
- 平台自身不可用、MCP 不可用、Key 无法解析，或平台返回的远端/分支与当前工作区冲突时，必须停止平台驱动流程，说明原因；普通业务项目退回 `本地 Harness 模式`，明确采用平台建设版本、自举平台能力或建设平台类治理能力的项目才可退回 `平台自身建设模式`。
- 退回本地文件闭环时，只能把文档落到本地 `docs/specs`，不得伪造需求平台回写结果。
...
- 流程模式：`需求平台需求设计模式`、`需求平台开发模式`、`项目接入初始化模式`、`本地 Harness 模式` 或 `平台自身建设模式`。
- 需求 Key：没有 Key 时写“无，本地平台建设”。
- 平台关联远端：需求平台返回的 Git 远端；本地 Harness 模式和平台自身建设模式可写当前仓库远端或“未配置”。
