...
- MCP 接入模式下必须先输出需求可行性评估和风险判断，结论类型至少包含“可继续设计、需澄清、需调整、暂不可实现”，并通过 MCP `upload_requirement_assessment` 回写平台；结论需要需求人补充、调整或暂不可实现时，停止在反馈阶段。
...
- MCP 接入模式下，每次执行、返修或补验后都要更新同一个 `execution-report.md`，并通过 MCP `upload_execution_report` 回写新版本。
...
- MCP 接入模式下，每次 Review、返修复审或补充验收后都要更新同一个 `review-report.md`，并通过 MCP `upload_review_report` 回写新版本。
