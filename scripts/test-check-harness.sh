...
make_root() {
  root=$1
  rm -rf "$root"
  mkdir -p "$root/docs/ai-harness/contracts" "$root/docs/ai-harness/decisions" "$root/docs/ai-harness/modules" "$root/docs/process" "$root/docs/runbooks" "$root/docs/specs/active/demo"
  ...
  printf '%s\n' '# meta' '' '- 状态：executing' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
  printf '%s\n' '# requirement' '' '- AC-001: demo' > "$root/docs/specs/active/demo/requirement.md"
  printf '%s\n' '# plan' '' '- AC-001: L0 L1 L2 L3 L4' > "$root/docs/specs/active/demo/plan.md"
}
...
make_init_root() {
  ...
  printf '%s\n' '# requirement' '' '- AC-999: intentionally incomplete during init' > "$root/docs/specs/active/demo/requirement.md"
}
...
make_root "$root"
rm "$root/docs/specs/active/demo/meta.md"
expect_fail "$root" complete "missing spec meta" "缺少文件"

make_root "$root"
printf '%s\n' '# meta' '' '- 状态：unknown' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
expect_fail "$root" complete "invalid spec status" "状态"

make_root "$root"
printf '%s\n' '# meta' '' '- 状态：planning' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
expect_fail "$root" complete "status role mismatch" "状态和当前角色不匹配"

make_root "$root"
printf '%s\n' '# requirement' '' '- AC-001: demo' '- AC-002: another demo' > "$root/docs/specs/active/demo/requirement.md"
expect_fail "$root" complete "plan misses acceptance id" "AC-002"

make_root "$root"
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：通过' > "$root/docs/specs/active/demo/review-report.md"
...
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：待定' '' '- AC-001：未验证' > "$root/docs/specs/active/demo/review-report.md"
printf '%s\n' '# meta' '' '- 状态：review' '- 当前角色：Review Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
expect_fail "$root" review "invalid review conclusion" "Review 结论必须"

make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# meta' '' '- 状态：complete' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
expect_fail "$root" complete "complete without review report" "缺少 Review 报告"

make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' '' '当前执行 agent 实测：环境不可达。' > "$root/docs/specs/active/demo/execution-report.md"
...
make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' '' '旧结论：环境不可达 是错误结论，已由实测覆盖。' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：通过' '' '- AC-001：通过' > "$root/docs/specs/active/demo/review-report.md"
printf '%s\n' '# meta' '' '- 状态：complete' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
sh "$CHECK_SCRIPT" "$root" complete >"$OUT_FILE" 2>&1

make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' '' '## 运行态证据' '' '- 执行目录：/workspace/repo' '- 启动命令：sh run.sh' '- profile/env/mode：test' '- 检查命令：curl http://localhost:8080/health' '- 原始错误摘要：connection refused' '- 是否代表用户环境：否，仅代表当前执行 agent 环境' '' '当前执行 agent 所在环境实测：环境不可达。' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：通过' '' '- AC-001：通过' > "$root/docs/specs/active/demo/review-report.md"
printf '%s\n' '# meta' '' '- 状态：complete' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
sh "$CHECK_SCRIPT" "$root" complete >"$OUT_FILE" 2>&1

make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' '' '## 运行态证据' '' '- 执行目录：/workspace/repo' '- 启动命令：sh run.sh' '- profile/env/mode：test' '- 检查命令：curl http://localhost:8080/health' '- console/network 错误摘要：connection refused' '- 是否代表用户环境：否，仅代表当前执行 agent 环境' '' '当前执行 agent 所在环境实测：环境不可达。' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：通过' '' '- AC-001：通过' > "$root/docs/specs/active/demo/review-report.md"
printf '%s\n' '# meta' '' '- 状态：complete' '- 当前角色：Execution Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
sh "$CHECK_SCRIPT" "$root" complete >"$OUT_FILE" 2>&1
...
make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# review' '' '## Review 结论' '' '- 结论：阻断' '' '## 返修交接清单' '' '| 修复 ID | 严重级别 | 关联验收 ID | 问题 | 修复要求 | 验证要求 |' '|---|---|---|---|---|---|' '| RF-001 | 阻断 | AC-001 | demo | fix | sh test.sh |' > "$root/docs/specs/active/demo/review-report.md"
printf '%s\n' '# meta' '' '- 状态：review' '- 当前角色：Review Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
...
make_root "$root"
printf '%s\n' '# execution' '' '## 验证结果' '' '- AC-001 命令：sh test.sh' > "$root/docs/specs/active/demo/execution-report.md"
printf '%s\n' '# meta' '' '- 状态：planning' '- 当前角色：Plan Agent' '- 当前分支：feature/demo' '- companion 仓库：无' > "$root/docs/specs/active/demo/meta.md"
