...
如果任务由多个 agent 或工具协作完成，必须按 `docs/process/agent-workflow.md` 使用文件交接：MCP 接入模式下计划 agent 先写并回写需求可行性评估，结论允许继续后只写清 `requirement.md`；本地 Harness 模式下使用同一套 `docs/specs` 文件和阶段门禁，但不伪造 MCP 回写。执行 agent 先基于最终确认的需求设计生成或更新 `plan.md`，再按计划实现并写 `execution-report.md`，review agent 只审查并写 `review-report.md`。
